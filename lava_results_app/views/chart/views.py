...
class OtherChartView(LavaView):
    def get_queryset(self):
        ...
        # of any group.
        other_charts = Chart.objects.filter(
            is_published=True, chart_group=None
        ).order_by("name")
...
class GroupChartView(LavaView):
    ...
    def get_queryset(self):
        ...
        group_charts = Chart.objects.filter(
            is_published=True, chart_group=self.chart_group
        ).order_by("name")
...
@BreadCrumb("Edit {name}", parent=chart_detail, needs=["name"])
@login_required
@ownership_required
