...
then
  ...
  docker inspect $BASE_IMAGE 2>/dev/null || docker pull $BASE_IMAGE 2>/dev/null || docker build -t $BASE_IMAGE docker/lava-server-base
  # Create a tag with the current version tag
  docker tag $BASE_IMAGE $BASE_IMAGE_NEW

  # Build the image
  echo "$IMAGE_TAG" > lava_common/VERSION
  DOCKER_BUILDKIT=1 docker build -t $IMAGE --build-arg base_image="$BASE_IMAGE_NEW" -f docker/lava-server/Dockerfile .
