...
then
  ...
  # Default values
  IMAGE_NAME=${IMAGE_NAME:-"hub.lavasoftware.org/lava/lava/amd64/lava-dispatcher"}
  ...
  DOCKER_BUILDKIT=1 docker build -t $IMAGE --build-arg base_image="$BASE_IMAGE_NEW" -f docker/amd64/lava-dispatcher/Dockerfile .
