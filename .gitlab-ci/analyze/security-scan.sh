...
then
  ...
  uv export --format cyclonedx1.5 --output-file sbom-uv.cdx.json
  ...
  syft dirs:. --output spdx-json > sbom.spdx.json
  ...
  grype sbom:sbom.spdx.json --output text > vuln-scan.txt
