...
start_lava_server_gunicorn() {
    ...
    if [ "$CAN_EXEC" = "1" ]; then
        exec /usr/bin/gunicorn3 lava_server.wsgi --config python:lava_server.gunicorn_gevent --log-level "$LOGLEVEL" --log-file - -u lavaserver -g lavaserver --preload --worker-class "$WORKER_CLASS" --workers "$WORKERS" --worker-tmp-dir /dev/shm $RELOAD $BIND $TIMEOUT $EXTRA_ARGS
    else
        setsid /usr/bin/gunicorn3 lava_server.wsgi --config python:lava_server.gunicorn_gevent --log-level "$LOGLEVEL" --log-file "$LOGFILE" -u lavaserver -g lavaserver --preload --worker-class "$WORKER_CLASS" --workers "$WORKERS" --worker-tmp-dir /dev/shm $RELOAD $BIND $TIMEOUT $EXTRA_ARGS &
        GUNICORN_PID=$!
    fi
}
