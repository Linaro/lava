...
class TestBzr(StdoutTestCase):  # pylint: disable=too-many-public-methods
    def setUp(self):
        ...
        # Create a Git repository with two commits
        subprocess.check_output(  # nosec - unit test support.
            ["bzr", "init", "repo"], env=self.env, stderr=subprocess.STDOUT
        )
        os.chdir("repo")
        subprocess.check_output(  # nosec - unit test support.
            ["bzr", "whoami", "lava-ci@example.com"],
            env=self.env,
        ...
        subprocess.check_output(  # nosec - unit test support.
            ["bzr", "add", "test.txt"], env=self.env, stderr=subprocess.STDOUT
        )
        subprocess.check_output(  # nosec - unit test support.
            ["bzr", "commit", "test.txt", "-m", "First commit"],
            env=self.env,
            stderr=subprocess.STDOUT,
        )
        with open("second.txt", "w") as datafile:
            datafile.write("Some more data")
        subprocess.check_output(  # nosec - unit test support.
            ["bzr", "add", "second.txt"], env=self.env, stderr=subprocess.STDOUT
        )
        subprocess.check_output(  # nosec - unit test support.
            ["bzr", "commit", "second.txt", "-m", "Second commit"],
            env=self.env,
            stderr=subprocess.STDOUT,
        )
