...
def parse_job_file(options: argparse.Namespace, logger: YAMLLogger) -> Job:
    ...
    device = DeviceDict.from_path(options.device)
    parser = JobParser()
...
def terminating_handler(*_: object) -> NoReturn:
    """
...
def main() -> int:
    ...
    try:
        ...
        if options.token is not None:
            logger.secrets_mask.add(options.token)

        # add job_id to environment, useful for user_commands
        os.environ["LAVA_JOB_ID"] = options.job_id
