                ...
                try:
                    ...
                    if compression == 'bz2':
                        if decompressor.unused_data:
                            ...
                            self.logger.exception(str(eof_exc))
                            raise JobError(eof_exc)
                    else:
                        self.logger.exception(str(eof_exc))
                        raise JobError(eof_exc)
                except (IOError, lzma.error, zlib.error) as z_exc:  # pylint: disable=no-member
                    self.logger.exception(str(z_exc))
                    raise JobError(z_exc)
