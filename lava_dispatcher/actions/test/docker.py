...
class DockerTestSetEnvironment(GetBoardId):
    ...
    def run(self, connection, max_end_time):
        ...
        if test_delay := self.job.device.get("character_delays", {}).get("test"):
            environment["LAVA_TEST_CHARACTER_DELAYS"] = test_delay
...
class DockerTestShell(TestShellAction, GetBoardId, DeviceContainerMappingMixin):
    ...
    def run(self, connection, max_end_time):
        ...
        container = f"lava-docker-test-shell-{self.job.job_id}-{self.level}"
        prefix = self.job.parameters.get("dispatcher", {}).get("prefix", "")
        if prefix:
            container = (
                f"lava-docker-test-shell-{prefix}-{self.job.job_id}-{self.level}"
            )
