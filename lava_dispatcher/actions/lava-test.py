...
class cmd_lava_test_install(BaseAction):
    ...
    """
    def run(self, tests, oot_test_container = None,oot_test_def_urls = None, timeout=2400):
        client = self.client

        _setup_testrootfs(client)
        _install_lava_test(client)

        if oot_test_container:
            client.run_shell_command("chroot /mnt/root apt-get -y install python-pip", response=MASTER_STR)
            client.run_shell_command("chroot /mnt/root pip -e " + oot_test_container, response=MASTER_STR)

        if oot_test_def_urls:
            for test_def_url in oot_test_def_urls:
