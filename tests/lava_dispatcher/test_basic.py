...
class LavaDispatcherTestCase(unittest.TestCase):
    ...
    @contextmanager
    def collect_lava_logs(self, action_or_job: Action | Job) -> list[tuple[str, str]]:
        log_records: list[LavaLogRecord] = []

        try:
            with patch.object(action_or_job.logger, "log_message") as log_message_mock:
                yield log_records
        finally:
            for call in log_message_mock.call_args_list:
                level_name, message, *_ = call.args
                log_records.append((level_name.upper(), message))
    ...
    def create_simple_job(
        self,
        device_dict: dict[str, Any] | None = None,
...
class Factory:
    ...
    def render_device_dictionary(
        self, device_name: str, job_context: dict[str, Any] | None = None
    ) -> str:
    ...
    def create_custom_job(
        ...
        env_dut: dict[str, Any] | None = None,
        *,
        device_dict_preprocessor: Callable[[dict[str, Any], None]] | None = None,
    ):
        if validate:
    ...
    def create_job(
        ...
        *,
        device_dict_preprocessor: Callable[[dict[str, Any], None]] | None = None,
        job_dict_preprocessor: Callable[[dict[str, Any], None]] | None = None,
    ):
