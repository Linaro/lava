...
class TestSocket:
    ...
    def send(self, data):
        if self.header:
            ...
            assert (  # nosec - unit test support
                json_data["response"] == self.response["response"]
            )
            self.passes += 1
            if self.message:
    ...
    def prepare(self, name):
        ...
        if self.response:
            self.log.info(
                "\tCoordinator: expecting a response: '%s'" % self.response["response"]
            )
...
class TestCoordinator:
        ...
        if "waitrole" in json_data:
            for client in self.group["roles"][json_data["role"]]:
                ...
                if client in self.group["waits"]:
                    logging.debug(
                        "replying: %s for %s"
                        % (self.group["waits"][client][messageID], client)
                    )
    ...
    def lavaSend(self, json_data, client_name):
        ...
        logging.debug(
            "message %s for %s"
            % (json.dumps(self.group["messages"][client_name][messageID]), client_name)
        )
        ...
        for client in self.group["clients"]:
            ...
            logging.debug(
                "broadcast %s for %s"
                % (json.dumps(self.group["messages"][client][messageID]), client)
            )
